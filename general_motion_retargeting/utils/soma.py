...
def load_soma_bvh_file(bvh_file):
    ...
    #
    # SOMA BVHs use a Z-forward / Y-up world frame. Rotate it so the character's
    # forward direction maps to GMR +X, matching G1's rest-pose forward axis.
    # The SOMA IK config rot_offsets are calibrated against this convention.
    ...
    # Unlike LAFAN1 / Nokov, SOMA's LeftFoot / RightFoot frames already carry
    # the ankle orientation expected by the IK config, so no FootMod target is
    # synthesized here.
